...
- **Frontend Components:** `pages/InstagramAcquisition.tsx` (stats cards, URL submit, per-stage `JobProgress`, source metadata header with inline `<video>` player (streams `video.mp4` once downloaded, falls back to thumbnail before that), frame + classification-badge gallery with OCR snippets, auto/manual OCR, inline question review with approve/reject/save/delete, JSON/CSV/SQLite export). Registered in `modules.ts` (Sources group) and `App.tsx` (`/instagram`).
...
- **APIs:** `/api/sources/instagram/`: `POST jobs`, `GET jobs`, `GET stats`, `GET jobs/{id}`, `DELETE jobs/{id}`, `GET jobs/{id}/frames`, `GET jobs/{id}/video` (token; streams the downloaded `video.mp4`), `POST jobs/{id}/analyze`, `GET frames/{id}/image` (token), `POST jobs/{id}/ocr`, `GET jobs/{id}/questions`, `PUT/DELETE questions/{id}`, `POST questions/{id}/approve|reject`, `GET jobs/{id}/export?format=json|csv|sqlite`.
